...
export const counter = ({
  ...
  if (sortStatus !== "asc") {
    return (
      +allData?.total - (+allData?.current_page - 1) * +allData?.per_page - _id
    );
  } else {
    return (+allData?.current_page - 1) * +allData?.per_page + _id + 1;
  }
};
...
export const formattedPrice = (inputPrice: string | number) => {
  const cleanPrice = inputPrice.toString().replace(/\D/g, "");
  return cleanPrice.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
};
export const unFormattedPrice = (inputPrice: string | number) => {
  return inputPrice.toString().replaceAll(",", "");
};
